...
def run_setup():
    ...
    print_box(
        "🌐 ШАГ 4: Прокси (опционально)",
        [
            "Прокси повышает безопасность при работе с Starvell",
            "Этот шаг можно пропустить и настроить позже через бота",
            "",
            "Поддерживаемые типы: HTTP, HTTPS, SOCKS5"
        ]
    )
    
    use_proxy = ask_yes_no("Хотите настроить прокси?", default=False)
    ...
    if use_proxy:
        print()
        print_info("Введите параметры прокси:\n")
        
        proxy_ip = ask("IP адрес прокси")
        proxy_port = ask("Порт прокси")
        
        print()
        has_auth = ask_yes_no("Требуется авторизация?", default=False)
        ...
        if has_auth:
            print()
            proxy_login = ask("Логин для прокси")
            proxy_password = ask("Пароль для прокси", secret=True)
        
        print()
        proxy_check = ask_yes_no("Проверять прокси перед использованием?", default=True)
        ...
        proxy_config = {
            ...
            'check': '1' if proxy_check else '0'
        }
        
        animate_dots("Настройка прокси")
        print_success("Прокси настроен!\n")
    else:
        print()
        print_info("Прокси отключен (можно включить позже)\n")
    ...
    print_box(
        ...
        [
            ...
            f"✓ Прокси: {'✓ Настроен' if use_proxy else '✗ Отключен'}",
